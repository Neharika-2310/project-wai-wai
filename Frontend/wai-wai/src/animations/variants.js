...
export const slideIn = (direction = "left", delay = 0, distance = 32, duration = 0.6) => {
  const axis = direction === "left" || direction === "right" ? "x" : "y";
  const sign = direction === "left" || direction === "up" ? -1 : 1;
  return {
    initial: { opacity: 0, [axis]: sign * distance },
    animate: { opacity: 1, [axis]: 0, transition: { duration, ease: [0.22, 1, 0.36, 1], delay } },
    exit: { opacity: 0, [axis]: sign * (distance / 2), transition: { duration: 0.3 } }
  };
};
